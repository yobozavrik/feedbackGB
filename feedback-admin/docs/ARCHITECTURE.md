...
| `/admin/users` + `/admin/audit` | User deactivate audit is written as `admin.user.deactivate`, but user activity drawer queries by `actor_user_id`; finding who deactivated a specific employee requires `target_user_id`. `v_audit_log` also lacks readable titles for `admin.user.create/update/deactivate/activate`, and `/admin/audit` only loads last 500 rows. | Core audit data is present for normal soft-delete (`users.is_active=false`), but the UI was optimized for "what this user did", not "what happened to this user". The 500-row cap keeps the audit page cheap. | Add target-user audit view/filter or a "who changed this employee" drawer, map all admin user lifecycle actions in `v_audit_log`, and consider server-side audit search/pagination beyond last 500 rows. Also audit the rollback path in `POST /api/admin/users` where failed PIN setup physically deletes the just-created user without `logAudit()`. |
