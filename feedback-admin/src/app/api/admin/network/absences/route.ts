...
export async function POST(req: Request) {
  ...
  if (!UUID.test(String(employeeId)) || !TYPES.has(String(type)) || !DATE.test(String(starts)) || !DATE.test(String(ends)) || String(starts) > String(ends) || (storeId != null && (!Number.isInteger(storeId) || Number(storeId) < 1)) || (note && (note.length < 3 || note.length > 500))) return fail("Недійсні дані відсутності", 400, "invalid_absence");
