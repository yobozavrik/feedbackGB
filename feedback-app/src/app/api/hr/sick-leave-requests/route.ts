...
/**
 * GET /api/hr/sick-leave-requests
 * Returns the CURRENT user's own hr_question/sick-leave requests, most
...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .eq("fields->>hr_topic", "sick-leave")
    .order("created_at", { ascending: false })
    .limit(20);

  if (error) {
    console.error("sick-leave-requests error", { code: error.code });
