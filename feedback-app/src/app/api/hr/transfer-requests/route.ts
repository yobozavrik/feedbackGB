...
/**
 * GET /api/hr/transfer-requests
 * Returns the CURRENT user's own hr_question/transfer requests, most
...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .eq("fields->>hr_topic", "transfer")
    .order("created_at", { ascending: false })
    .limit(20);

  if (error) {
    console.error("transfer-requests error", { code: error.code });
